"""Pytest configuration for eval suite.

This file is automatically loaded by pytest before running evals.
"""
...
# Default model for evals
DEFAULT_TEST_MODEL = "gpt-4o-mini"

# Stats file for historical analysis
STATS_FILE = PROJECT_ROOT / "out" / "eval_stats.jsonl"


def pytest_addoption(parser):
    """Add custom command-line options for extraction evals."""
    parser.addoption(
        "--models",
        action="store",
...
def pytest_collection_finish(session):
    """Announce stats file location when running evals."""
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    """Print timing summary at the end of eval run."""
    if not _timing_reports:
        return
